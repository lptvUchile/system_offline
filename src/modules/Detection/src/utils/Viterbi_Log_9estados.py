...
def Viterbi_Log_SIL3(P_Transicion, P_Inicial, P_Observacion):
    ...
    N_modelos = len(P_Inicial)
    N_frames = len(P_Observacion[0])
        ...
        for j in range(N_modelos):
            ...
            if i == 0:
                ...
                for k in range(N_estados):
                    ...
                    Probs_Dur_Es = Restricciones_Duracion_Estado(
                        Token[i - 1], 12, [j, k]
                    )

                    Probs_Dur_Ev = Restricciones_Duracion_Evento(
                        i, N_frames, k, j, Token_eventos[i - 1], 12
                    )

                    P_ML = Probabilidad_ML(i, j, k, N_frames, N_estados, 12)
                    P_Transicion_final = (
                        P_Transicion[j][k] + P_ML + Probs_Dur_Ev + Probs_Dur_Es
                    )
                    max_arg = (
                        np.array(list(itertools.chain(*Delta[i - 1][:])))
                        + P_Transicion_final
                    )
                    ...
                    Token, Token_eventos = Guardar_Token(
                        i, j, k, Coordenada, Token, Token_eventos
                    )
        ...
        if i == N_frames - 1:
            ...
            for j in range(N_modelos):
                ...
                for k in range(N_estados):
                    ...
                    Probs_Dur_Ev = Restricciones_Duracion_Evento(
                        i, N_frames, k, j, Token_eventos[i], 12
                    )
                    max_arg = (
                        np.array(list(itertools.chain(*Delta[i][:])))
                        + P_Transicion[j][k]
                        + P_ML
                        + Probs_Dur_Ev
                        + Probs_Dur_Es
    ...
    S_opt = Backtraking(
        N_frames, Delta, Psi, Delta[N_frames][0][2], Delta[N_frames][1][8]
    )
