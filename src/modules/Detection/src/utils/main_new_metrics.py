...
def metricas_viterbi(file_viterbi, ref_file_p, nombre_conjunto):
    ...
    viterbi_p = pd.read_csv(
        file_viterbi, sep=" ", names=["utt", "unos", "ini", "fin", "label"]
    )
    ...
    for i in range(len(thresholds)):
        ...
        metrics_per_evento = metrics_per_evento.append(
            {
                "score": "1",
        ...
        Metricas.to_csv(
            path_umbral + "/" + "score_metricas_" + nombre_conjunto + ".csv",
            sep="\t",
            index=[0, 1, 2],
        )
        results1.to_csv(
            path_umbral + "/" + "score_" + nombre_conjunto + ".csv",
            sep="\t",
            index=False,
        )
        tfile = open(
            path_umbral + "/" + "score_" + "FP_FN_" + nombre_conjunto + ".txt", "a"
        )
        ...
        metrics_per_evento.to_csv(
            path_resultados + "/resume_metrics_events_" + nombre_conjunto + ".csv",
            sep="\t",
            index=False,
        )
