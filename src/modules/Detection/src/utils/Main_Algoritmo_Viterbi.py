...
def Algoritmo_Viterbi(
    file_viterbi, Probs_Observacion, path_3estados, path_transitions_file
):

    # phones = "Detection/models/phones_3estados.txt"
    phones = path_3estados
    transitions_file = path_transitions_file
    # transitions_file = "Detection/models/final_16_layers3_s1_lr001_NorthChile.mdl"
    ...
    topology.close()
    P_Transicion, Vocab = Prob_Transicion_automatico(
        Vocabulario, lineas, Phones_lineas, "mono"
    )
    ...
    for Indice in range(len(Probs_Observacion)):
        ...
        Escribir_ctm(Ruido_diff, Evento_diff, Indice, fs)
