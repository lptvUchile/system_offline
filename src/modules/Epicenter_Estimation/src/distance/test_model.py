...
from src.utils import (MyBatchGenerator_lstm_mlp, compute_binary_metrics, save_binary_metrics, 
                       ...
                       new_binary_metrics)
from src.distance.model import DistanceModel
...
path_feat_in_test_cnn = f'./data/features/distance/{prefix}/{test_folder_name}/{test_name}/feat_lstm_raw_test_'+feat_type+'.npy'.replace('\\', '/')
...
path_labels_test = f'./data/features/distance/{prefix}/{test_folder_name}/{test_name}/distance_raw_test_'+feat_type+'.npy'.replace('\\', '/')
...
# LOAD FEATURES
features_dir = os.path.join('data','features','distance',args.model_prefix, model_test_folder_name, model_test_name)
...
test_target = labels_test['distance']

X_test_cnn, X_test_mlp = pad_and_convert_to_tensor(feat_in_test_cnn),tf.convert_to_tensor(feat_in_test_mlp, dtype=tf.float32)

#LOAD MODEL
model_path = os.path.join("models","distance",args.model_prefix, model_test_folder_name, model_test_name,f"{args.seed}.h5")
